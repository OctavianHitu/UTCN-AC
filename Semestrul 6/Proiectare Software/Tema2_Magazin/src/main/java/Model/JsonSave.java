...
public interface JsonSave {
    ...
    default void savejson(List<IncaltaminteMagazin> incaltainteMagazins) throws IOException {
        FileWriter newfile = new FileWriter(pathfile);
        JSONObject object;

        for (IncaltaminteMagazin inc :incaltainteMagazins)
        {
            ...
            for (Incaltaminte in: inc.getIncaltamintes())
            {
                ...
                papuc.add("Exemplare Vandute:" +in.getIncaltaminteDisponibila().getNumarexemplarevandute());
